...
if [ $ARG == "start" ]; then
    ...
    #nohup /usr/local/bin/nlsr -f conf-files/lbl/nlsr-lbl.conf &
#    /usr/local/bin/nfdstat_c -p /cmip5/den/ndnmap/stats -d 1 &
    ...
    #nohup /usr/local/bin/ndn-repo-ng -c $DIR/conf-files/lbl/repo-ng-d.conf &
    #nohup /usr/local/bin/ndn-repo-ng -c conf-files/lbl/repo-ng-d.conf &
fi
